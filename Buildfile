...
APPLICATION_NAME="atlassian-crowd"

APPLICATION_RSS="https://my.atlassian.com/download/feeds/crowd.rss"
...
function test_container {
  case $APPLICATION_BRANCH in
    master)
      docker run -t --rm --env-file files/environment.list ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:master rake spec /home/crowd/spec
    ;;
    develop)
      docker run -t --rm --env-file files/environment.list ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:develop rake spec /home/crowd/spec
    ;;
    release*)
      APPLICATION_RELEASE="$(git symbolic-ref --short HEAD | grep -o -E "(\d{1,2}\.){2,3}\d")"
      docker run -t --rm --env-file files/environment.list ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:${APPLICATION_RELEASE} rake spec /home/crowd/spec
    ;;
    feature*)
      GIT_HASH="$(git rev-parse --short HEAD)"
      docker run -t --rm --env-file files/environment.list ${DOCKER_REPOSITORY}/${APPLICATION_NAME}:feature-${GIT_HASH} rake spec /home/crowd/spec
    ;;
    *)
      echo "No match found"
    ;;
